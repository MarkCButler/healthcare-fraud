...
#     in one of the csv files.  The unique row identifier for each of this is
#     a column name.
#
# 2. inpatient_claims, outpatient_claims -- data frames that have ClaimID as a
#    unique row identifier and include a variety of features that characterize
#    individual claims.
#
...
# there is a concatenated data frame (named 'claims' for this example) that
# has a column identifying each row as inpatient or outpatient.  Note that in
# some cases, certain columns of the "inpatient frame" or the "outpatient
# frame" are dropped in order to perform the concatenation.
...
get_patient_claim_counts <- function(claim_data, claim_type,
    ...
    claim_counts <- mutate(claim_counts, claim_type = !!claim_type)
...
augment_claim_data <- function(claim_data, patient_data, claim_counts,
    ...
    claim_data <- left_join(claim_data, patient_data, by = 'BeneID') %>%
        mutate(patient_age = (ClaimStartDt - DOB) / 365.24) %>%
...
get_doctor_data <- function(claim_data, claim_type, training = TRUE) {
    ...
    doctor_data <- mutate(doctor_data, claim_type = !!claim_type)
...
#
get_provider_claim_counts <- function(claim_data, claim_type, training = TRUE) {
    claim_counts <- claim_data %>%
        mutate(
...
provider_claim_counts <- rbind(provider_claim_counts_inpatient,
                               provider_claim_counts_outpatient,
                               make.row.names = FALSE)
