...
```{r, data-frames, message=FALSE, cache=TRUE}
source('globals.R')
...
for (column_name in chronic_conditions) {
    ...
    fig <- ggplot(patients, aes_string(x = 'visit_type', fill = column_name)) +
...
in_out_providers_vec <- intersect(inpatient_providers_vec, outpatient_providers_vec)
...
# Column names associated with payments, along with corresponding labels for
# plots.
...
fig <- ggplot(inpatient_claims, aes(x = age, y = visit_duration)) +
    geom_point(color = 'navyblue') +
...
for (visit_type in visit_types) {
    ...
    title <- str_c('Insurer payments, ', visit_type, ' visits')
    fig <- plot_payment_distribution(to_plot, InscClaimAmtReimbursed, title)
    print(fig)

    title <- str_c('Deductible, ', visit_type, ' visits')
    fig <- plot_payment_distribution(to_plot, DeductibleAmtPaid, title)
    print(fig)

    title <- str_c('Insurer payment vs deductible, ', visit_type, ' visits')
    fig <- ggplot(to_plot,
                  aes(x = DeductibleAmtPaid, y = InscClaimAmtReimbursed)) +
        geom_point(color = 'navyblue') +
    ...
    title <- str_c('Percent covered by insurer, ', visit_type, ' visits')
    fig <- ggplot(to_plot, aes(percent_covered)) +
...
for (visit_type in visit_types) {
    to_plot <- claim_durations %>%
        filter(visit_type == !!visit_type)

    title <- str_c('Mean claim duration by provider, ', visit_type, ' visits')
    fig <- ggplot(to_plot, aes(x = PotentialFraud, y = mean_claim_duration)) +
...
for (visit_type in visit_types) {
    to_plot <- claims %>%
        filter(visit_type == !!visit_type, visit_cost != 0) %>%
        select(Provider, PotentialFraud, visit_cost, claim_duration) %>%
        mutate(
            claim_cost_per_day = visit_cost / as.numeric(claim_duration + 1)
        )

    title <- str_c('Claim amount vs claim duration, ', visit_type, ' visits')
    fig <- ggplot(to_plot, aes(x = claim_duration, y = visit_cost)) +
    ...
    fig <- to_plot %>%
        ggplot(aes(x = claim_cost_per_day)) +
        geom_histogram(
            aes(fill = PotentialFraud), position = 'identity',
            bins = 40, alpha = 0.15
        ) +
        geom_freqpoly(
            aes(color = PotentialFraud), position = 'identity',
            bins = 40, size = 0.7
        ) +
        ylab('Number of claims') +
        log_scale_dollar('Cost per day', 'x') +
        labs(fill = legend_title, color = legend_title) +
        ggtitle(title)
    print(fig)
}
...
to_plot <- filter_out_no_codes(inpatient_duplicates)
title_count_identical <- 'Number of duplicates submitted for '
fig_base <- ggplot(to_plot,
                  aes(x = as.factor(identical_claim_count - 1))) +
...
fig_base <- ggplot(to_plot,
              aes(x = as.factor(identical_claims_per_provider))) +
    xlab('Number of claims per provider') +
    ggtitle(str_c(title_per_provider, 'inpatients'))
plot_bar_charts(fig_base, 'claims')

to_plot <- filter_out_no_codes(outpatient_duplicates)
fig_base <- ggplot(to_plot,
              aes(x = identical_claim_count - 1)) +
...
fig_base <- ggplot(to_plot,
              aes(x = identical_claims_per_provider)) +
...
plot_fraction_original <- function(duplicates_data, visit_type) {
...
cat(message)
plot_fraction_original(inpatient_duplicates, 'inpatient')
plot_fraction_original(outpatient_duplicates, 'outpatient')
```
</div>
