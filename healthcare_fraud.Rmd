...
For patients, however, there is a small but still significant fraction that
has 50% fraudulent claims.  There may be a distinct fraud mechanism associated
with this smaller group.

Note that in the category of inpatients having only a single claim, the median
percentage of fraudulent claims is 100%.  (About 57% of the inpatients with
only a single claim are associated with a fraudulent provider.)
...
Plots in the tabbed subsections show that inpatient visit duration depends on
patient age and the number of chronic conditions a patient has.  But there
isn't a strong association between fraud and visit duration.
...
in which the patient was not formally admitted as an inpatient.
  ...
  - The claim duration is identical to the visit duration for most inpatients.
  - The mean claim duration for outpatients is 2.4 days.

Similarly to visit duration, claim curation does not have a strong association
with fraud.
...
### Claims in identical groups {.tab-display .unnumbered}
...
in a time-series decomposition of daily visit counts.
...
```{r, series-summary, echo=FALSE, cache=TRUE}
...
time-series decomposition will include only the data in these ranges.
...
data.  Further analysis would be needed to determine whether these
seasonalities are just random variations.
...
Are these potentially fraudulent providers duplicating their own medical
...
so, these fraudulent IDs (or if the doctors or patients willingly participate)
