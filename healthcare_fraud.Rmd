...
The training data was used for the analysis.  Details of the data frames:
  ...
  - The patients who made these hospital visits (25 columns, <nobr>~ 140,000</nobr>
    rows)
  - The hospitals visited, referred to as providers (2 columns:  provider ID and
    PotentialFraud label, <nobr>~ 5,400</nobr> rows)
...
payments to hospitals.  The dataset appears to have information only about
payments to hospitals.
  ...
  - In the dataset, the information about payment is contained in the features
    InscClaimAmtReimbursed and DeductibleAmtPaid.  Breaking down payments by
    amount reimbursed and deductible paid is natural for hospital services, but
    the payments made by patients for doctor services are distinct from these
    two categories.
...
However Medicare defines an outpatient as one who has not formally been admitted
to a hospital with a doctor's order.  A
...
significant for the patient, because inpatient and outpatient visits are covered
by different parts of the Medicare program (Part A and Part B, respectively.)
  ...
  - However, in general a copayment must be made for each outpatient visit, and
    ...
    that the one-time deductible for inpatient visits.  (In appears that in the
    csv file for outpatient visits, the feature DeductibleAmtPaid gives
    patient copayments.)
...
Each hospital visit is associated with a claim made by a provider.  For purposes
of exploratory data analysis, claims made by a provider that has been flagged
for potential fraud also receive the "potential fraud" label.
...
## Visit / claim durations

### Inpatient visit duration {.tabset .unnumbered}

#### Summary {.unnumbered}

The 36-day bound on visit durations is striking.  I didn't find any indication
that 36 days is a significant threshold for visit durations covered by
Medicare.
...
#### Distribution {.tab-display .unnumbered}
...
#### By age {.tab-display .unnumbered}

<div class="code-block">

```{r, show-visit-duration, ref.label='visit-duration', eval=FALSE}
```

```{r, visit-duration, echo=FALSE, cache=TRUE}
...
### By chronic condition {.tab-display .unnumbered}
...
fig <- to_plot %>%
    ...
    labs(y = 'Number of claims', fill = '', color = '') +
