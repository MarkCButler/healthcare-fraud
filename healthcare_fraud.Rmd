...
for (claim_type in claim_types) {
    ...
    fig <- ggplot(to_plot, aes(x = patient_claim_count)) +
...
plot_chron_cond_claim_counts(patients)
```
...
for (claim_type in claim_types) {
    ...
    fig <- ggplot(to_plot, aes(x = patient_age)) +
...
for (claim_type in claim_types) {
    ...
    vec_labels <- convert_to_labels(names(to_plot_vec)) %>%
        factor() %>%
        reorder(to_plot_vec)
    to_plot <- data.frame(label = vec_labels, patient_claim_count = to_plot_vec)
    title <- str_c('Number of visits, ', claim_type, 's')
    fig <- ggplot(to_plot, aes(x = label, y = patient_claim_count)) +
...
fig <- ggplot(inpatient_claims, aes(x = patient_age, y = visit_duration)) +
    geom_point(color = 'navyblue') +
...
for (claim_type in claim_types) {
    to_plot <- provider_claim_durations %>%
        filter(claim_type == !!claim_type)
...
for (claim_type in claim_types) {
    to_plot <- claims %>%
        filter(claim_type == !!claim_type, visit_cost != 0) %>%
